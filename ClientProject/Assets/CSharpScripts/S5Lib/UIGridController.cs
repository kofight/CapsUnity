...
public class UIGridController<TItem> where TItem : UIGridItem, new()
{
    ...
    {
        if (TotalPageNum_Prop == 0)
            return;
        if (TotalPageNum_Prop == m_currentIndex + 1)
            return;
        if (TotalPageNum_Prop == 1)
            return;
        HideOldPage();
        m_currentIndex++;
        ShowNewPage();
    ...
    public void LastPage()
    {
        if (TotalPageNum_Prop == 1)
            return;
        HideOldPage();
        m_currentIndex = TotalPageNum_Prop - 1;
        ShowNewPage();
        Reposition();
    }
    ...
    {
        for (int i = m_currentIndex * PageSize_Prop; i < PageSize_Prop * (m_currentIndex + 1); i++)
        {
            if (i >= m_GridItems.Count)
                break;
            m_GridItems[i].GameObject_Prop.SetActive(true);
        }
    }

    private void HideOldPage()
    {
        for (int i = m_currentIndex * PageSize_Prop; i < PageSize_Prop * (m_currentIndex + 1); i++)
        {
            if (i >= m_GridItems.Count)
                break;
            m_GridItems[i].GameObject_Prop.SetActive(false);
        }
    }
    ...
    public UIGridController(UIWindowNGUI window, string gridName, string ItemName, string defaultBg = "")
    {
    ...
    public void SetDataSource(List<TItem> source, int pageSize = 0)
    {
        ...
        if (PageSize_Prop > 0)
        {
            TotalPageNum_Prop = source.Count % PageSize_Prop == 0 ? source.Count / PageSize_Prop : source.Count / PageSize_Prop + 1;
            m_currentIndex = 0;
            FirstPage();
        }
        else
